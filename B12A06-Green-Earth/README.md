...
var → Old way, function-scoped, can be redeclared, not very safe (can cause bugs).
...
forEach() → Loops through each item but does not return anything. Just executes code.
...
Destructuring allows you to easily extract values from arrays or objects.
...
const person = { name: "Liya", age: 22 };
const { name, age } = person;
console.log(name, age); // Liya, 22

5) Template literals in ES6 (vs. string concatenation)

Template literals use *backticks ()* instead of quotes. You can insert variables or expressions directly inside with ${ }`.
